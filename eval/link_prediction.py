...
def get_link_score(fu, fv, operator):
    """
    Given a pair of embeddings, calculate the connection strength between nodes based on operator (such as Hadammad product, etc.)
    """
    fu = np.array(fu)
    fv = np.array(fv)
    if operator == "HAD":
...
def get_roc_score_t(edges_pos, edges_neg, source_emb, target_emb):
    """
    Calculate the ROC score, F1 score, and precision-recall AUC for positive and negative edges
    based on the embeddings of the source and target nodes.

    Parameters:
        edges_pos (list of tuples): Positive edge pairs for evaluation.
        edges_neg (list of tuples): Negative edge pairs for evaluation.
        source_emb (numpy.ndarray): Embeddings for the source nodes.
        target_emb (numpy.ndarray): Embeddings for the target nodes.

    Returns:
        tuple: Contains ROC score, F1 score, and precision-recall AUC.
    """
    def sigmoid(x):
        return 1 / (1 + np.exp(-x))
